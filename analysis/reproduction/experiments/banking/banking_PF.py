...
# --- load data ---
df1 = load_dataset("banking77")
np.unique(df1["train"]["label"])

label_str = [str(i) for i in df1["train"]["label"]]
df1 = pd.DataFrame({"txt": df1["train"]["text"], "label": label_str})

# remove documents with 0 tokens after tokenization
X = CountVectorizer(max_features=35000).fit_transform(df1.txt)
...
spf1 = SPF(keywords=keywords, residual_topics=len(np.unique(df1.label)))
spf1
...
# -- Train the model
spf1.model_train(lr=0.001, epochs=750, tensorboard=False, early_stopping=False, print_information=True)
...
true_labels = df1['label'].values
unique_categories = np.unique(true_labels)
assert len(unique_categories) == len(
    np.unique(df1.label)), "Number of unique categories must be K to match predictions 0-19"
